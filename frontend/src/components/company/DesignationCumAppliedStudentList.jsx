...
function DesignationCumAppliedStudentList({ jobId, onStudentClick }) {
    ...
    const fetchCompanyJobDetails = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.REACT_APP_BACKEND_URL}/api/v2/companies/get-applied-students-list/${jobId}`
        );
        setJob(res.data.data);
        const resDesignation = await axios.get(
          `${import.meta.env.REACT_APP_BACKEND_URL}/api/v3/companies/job/get-job-details/${jobId}`
        );
  ...
  const unHireStudents = async (jobId) => {
    try {
      await axios.get(`${import.meta.env.REACT_APP_BACKEND_URL}/api/v2/companies/unhire-all-student/${jobId}`);
  ...
  const hireStudent = async (studentId, jobId) => {
    try {
      await axios.get(`${import.meta.env.REACT_APP_BACKEND_URL}/api/v2/companies/hire-student/${studentId}/${jobId}`);
